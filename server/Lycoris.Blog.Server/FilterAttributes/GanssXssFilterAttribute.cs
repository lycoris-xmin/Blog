...
namespace Lycoris.Blog.Server.FilterAttributes
{
    ...
    public class GanssXssFilterAttribute : BaseActionAttribute
    {
        ...
        public override Task ActionHandlerBeforeAsync(ActionExecutingContext context)
        {
            ...
            var ps = context.ActionDescriptor.Parameters.Where(x => x.ParameterType.Equals(typeof(string)) || x.ParameterType.IsClass)
                                                        .Where(x => x.ParameterType.Equals(typeof(IFormFile)) == false)
                                                        .WhereIf(settings != null && settings.IgnoreProperties.HasValue(), x => !settings!.IgnoreProperties.Contains(x.Name)).ToList();
