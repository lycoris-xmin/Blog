...
import { getSystemSettings, showdocPushTest, saveShowdocSettings, saveSystemFileClear, saveSystemDbClear } from '../../../api/configuration';
...
const saveShowdoc = async () => {
  ...
  try {
    let res = await saveShowdocSettings(showdoc.host);
    if (res && res.resCode == 0) {
      toast.success('保存成功');
    }
  } finally {
